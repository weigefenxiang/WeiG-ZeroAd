...
final class CodeUpdater {
    ...
    static Available check(Context context) throws Exception {
        ReleaseClient.Match match = ReleaseClient.latestChannelWithAsset(
                BuildConfig.CODE_REPOSITORY, "update-manifest", ".json",
                BuildConfig.UPDATE_CHANNEL);
        ReleaseClient.Release release = match.release();
        File file = ReleaseClient.download(context, match.asset(), 1024L * 1024L);
        try {
